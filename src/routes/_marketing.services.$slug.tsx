...
const DATA: Record<string, {
  title: string; intro: string; makes: string[]; submit: string[]; materials: string[]; workflow: string[];
}> = {
  "fixed-restorations": {
...
function ServiceDetail() {
  const d = Route.useLoaderData() as (typeof DATA)[keyof typeof DATA];
  return (
    <div className="bg-white py-20">
      <div className="max-w-5xl mx-auto px-5 lg:px-8">
        <Reveal>
          ...
          <h1 className="mt-4 text-4xl md:text-5xl font-bold inline-block border-b-4 border-teal pb-2">{d.title}</h1>
          <p className="mt-6 text-lg text-muted-grey leading-relaxed max-w-3xl">{d.intro}</p>
        </Reveal>
