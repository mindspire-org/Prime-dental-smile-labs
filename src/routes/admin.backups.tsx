...
export default function BackupsPage() {
  ...
  async function runCleanup() {
    if (!confirm("This will permanently delete old data based on retention settings. Continue?")) return;
    setLoading(true); setMessage(""); setCleanupResult(null);
    try {
      const r = await apiFetch<any>("/api/admin/backups/cleanup", {
        method: "POST",
        body: JSON.stringify({ months: settings.retentionMonths, scope: settings.autoDeleteScope }),
      });
          ...
          <Card>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={settings.retentionMonths ?? 3}
                  onChange={e => setSettings((s: any) => ({ ...s, retentionMonths: Number(e.target.value) }))}
